#!/usr/bin/env python3
"""
Person Tracking Database for SQLite
Stores ALL person entries (both new and re-identified) for tracking history.
Separate from Vector Database which only stores new person embeddings.
"""
...
class PersonTrackingSQLiteDB:
    ...
    def _create_tables(self):
        """Create the person tracking table with the specified schema."""
        with self._get_db_conn() as conn:
            cursor = conn.cursor()
            
            # Create person_tracking table
            cursor.execute("""
            ...
            conn.commit()
            # print(f"✅ Person tracking tables created in {self.db_path}")
    
    def generate_unique_id(self):
        """Generate a unique UUID for the tracking entry."""
        return str(uuid.uuid4())
    
    def record_person_entry(self, person_id: int, entry_timestamp=None):
        """
    ...
    def record_person_exit(self, person_id: int, exit_timestamp=None, unique_id=None):
        """
    ...
    def get_person_tracking_history(self, person_id: int):
        """
    ...
    def get_all_tracking_entries(self, limit: int = 100):
        """
        Get all tracking entries with optional limit.
        
        Args:
            limit (int): Maximum number of entries to return
        
        Returns:
            list: List of all tracking records
        """
        with self._get_db_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("""
    ...
    def get_tracking_summary(self, person_id: int = None):
        """
    ...
    def clear_database(self):
        ...
        with self._get_db_conn() as conn:
            ...
            conn.commit()
        # print("✅ All person tracking data cleared.")
    
    def export_to_csv(self, output_path="person_tracking_data.csv"):
        ...
        # print(f"✅ Person tracking data exported to {output_path}")

# Example usage and testing
if __name__ == '__main__':
    # Initialize database
    tracking_db = PersonTrackingSQLiteDB()
    
    # Clear any existing data
    tracking_db.clear_database()
    
    # print("=== Testing Person Tracking SQLite Database ===")
    ...
    #     print(f"     Person ID {person_id}: {entry_time}")
    
    # print("\n9. Testing CSV export...")
    # Export to CSV
    tracking_db.export_to_csv()
    # print(f"   ✓ Data exported to test_person_tracking_export.csv")
