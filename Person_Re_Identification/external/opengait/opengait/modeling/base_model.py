...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        # Skip data loader creation for inference-only mode
        if not training:
            if 'transform' in cfgs['evaluator_cfg'] and cfgs['evaluator_cfg']['transform']:
                self.evaluator_trfs = get_transform(
                    cfgs['evaluator_cfg']['transform'])
            else:
                self.evaluator_trfs = []
        # Completely disable test loader creation for inference
        # elif self.engine_cfg['with_test']:
        ...
        # Handle single-process inference without distributed training
        try:
            self.device = torch.distributed.get_rank()
            torch.cuda.set_device(self.device)
            self.to(device=torch.device("cuda", self.device))
        except (ValueError, RuntimeError):
            # Single process inference - use default CUDA device
            self.device = 0
